  ...
  return (
    <group position={[0, 0, depth]}>
      <PlayerParticleSphere
        radius={radius}
        color={rgbaToHex(player.cell.color || "#ffffff")}
...
const GlobalPerspectiveView = ({
  ...
  const [audioStatus, setAudioStatus] = useState<"pending" | "ready" | "playing">(
    "pending"
  );
  ...
  useEffect(() => {
    ...
    console.log("[Global] resolveNoiseCraftEmbed() returned:", cfg);
  ...
  useEffect(() => {
    ...
    // 플레이어가 없을 때는 기본 파라미터 전송 (조용한 상태)
    if (!clusters.length) {
      const rawParams: NoiseCraftParam[] = [
        {
          nodeId: "220",
          paramName: "value",
          value: 0, // 첫 번째 클러스터 없음
        },
        {
          nodeId: "221",
          paramName: "value",
          value: 0, // 두 번째 클러스터 없음
        },
        {
          nodeId: "233",
          paramName: "value",
  ...
  useEffect(() => {
    ...
    const handleAudioGlobalV2 = (...args: unknown[]) => {
      ...
      if (grids) {
        // 디버깅: 그리드 상태 확인
        const bassActive = grids.bass?.flat().filter((v) => v === 1).length ?? 0;
        const baritoneActive = grids.baritone?.flat().filter((v) => v === 1).length ?? 0;
        const tenorActive = grids.tenor?.flat().filter((v) => v === 1).length ?? 0;
        if (bassActive > 0 || baritoneActive > 0 || tenorActive > 0) {
          // eslint-disable-next-line no-console
          console.log("[Global] Sequencer update:", {
            bass: bassActive,
            baritone: baritoneActive,
            tenor: tenorActive,
            nodeIds,
          });
        }
  ...
  const { geometry, velocities } = useMemo(() => {
    ...
    const playerColors =
      mark.players.length > 0
        ? mark.players.map((p) => new THREE.Color(rgbaToHex(p.cell.color || "#ffffff")))
