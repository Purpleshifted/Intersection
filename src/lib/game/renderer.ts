...
const renderPlayers = ({
  ...
  state.playerOrder.forEach((playerId, index) => {
    ...
    const isSelf = Boolean(player.isSelf);
...
const renderCollisionConnections = ({
  ...
  state.collisionLines.forEach((pair) => {
    ...
    if (state.mode === "personal") {
      ...
      const otherPlayer = isSelfA ? b : a;

      // 화성 호환성 가져오기 (0.0 ~ 1.0)
      const compatibility = otherPlayer.harmonicCompatibility ?? 0;
      // 호환성이 높을수록 더 강한 glow (최대 2배)
      const glowIntensity = 1.0 + compatibility; // 1.0 ~ 2.0
      const baseSteps = 6;
      const steps = Math.floor(baseSteps * glowIntensity); // 호환성에 따라 스텝 수 증가

      // 부드러운 외곽 글로우 (단색 흰색 계열, 중앙 코어 라인 없이)
      // 호환성이 높을수록 더 밝고 넓은 glow
      for (let i = 0; i < steps; i += 1) {
        const t = i / (steps - 1);
        // 호환성이 높을수록 더 밝게 (최대 2배)
        const alpha = 0.18 * (1 - t) * glowIntensity;
        const width = (10 + 18 * t) * glowIntensity;
        ...
        const inset = (8 + 16 * t) * glowIntensity;
...
export const renderScene = (params: RenderParams) => {
  ...
  const shouldRender = params.state.playing || (params.state.mode === "personal" && params.state.selfId && Object.keys(params.state.players).length > 0);
  if (shouldRender) {
