...
export const toPlayerSnapshot = (
  ...
  return {
    ...
    // 화성 호환성 정보 추가
    harmonicCompatibility: (player as any).harmonicCompatibility ?? 0,
    harmonicTargetId: (player as any).harmonicTargetId ?? null,
  };
};
...
export const mapServerPayloadToSnapshots = (params: {
  ...
  if (playerData) {
    pushSnapshot(playerData, {
      isSelf: mode === "personal" && Boolean(selfId && playerData.id === selfId),
