...
import { mapServerPayloadToSnapshots, toPlayerSnapshot } from "@/lib/game/mappers";
...
export const registerSocketEvents = ({
  ...
  const onConnect = () => {
    ...
    // 연결 시 playing 상태를 true로 설정 (welcome 이벤트 전에)
  ...
  const onWelcome = (
    ...
    console.log("[Mobile] onWelcome:", {
      mode,
      playerSettingsId: playerSettings.id,
      socketId: socket.id,
      currentSelfId: getState().selfId,
      resolvedSelfId: selfId,
    });
    ...
    if (mode === "personal" && typeof window !== "undefined") {
      ...
      // 난이도 정보 전송
      if (difficulty) {
        handshake.difficulty = difficulty;
      }
    }
  ...
  const onPlayerMove = (
    ...
    if (mode === "personal" && currentSelfId) {
      ...
      if (serverSelf) {
        ...
        // playerData 또는 userData에서 자기 플레이어 찾기
        let selfPlayerData = effectivePlayerData;
        if (!selfPlayerData || selfPlayerData.id !== currentSelfId) {
          // userData에서 찾기
          selfPlayerData = userData.find((p) => p.id === currentSelfId);
        }
        
        if (selfPlayerData && selfPlayerData.id === currentSelfId) {
          // playerData가 있지만 아직 players에 없는 경우 (첫 업데이트)
          const snapshot = toPlayerSnapshot(selfPlayerData, {
            isSelf: true,
            fallbackId: currentSelfId,
          ...
          if (snapshot) {
            ...
            if (!order.includes(currentSelfId)) {
              order.unshift(currentSelfId);
            }
          }
        }
      }
